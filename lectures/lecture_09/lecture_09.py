...
# TODO Uncomment
# print_slogan(slogan) # call function
...
def format_ingredients(cereal):
    return ', '.join(cereal[2]) # TODO implement

cereal_ingredients = []
for cereal in cereals[1:]:
    cereal_name = cereal[1]
    ingredients = format_ingredients(cereal)
...
def get_cereals_with_corn_syrup(cereals):
    ...
    for cereal in cereals:
        if has_corn_syrup(cereal): # delegates question to another function
            results.append(cereal[1])

    return results

# call function
cereals_with_corn_syrup = get_cereals_with_corn_syrup(cereals) # TODO call function
...
def get_cereals_by_ingredient(cereals, ingredient):
    cereals_with_ingredients = []
    for cereal in cereals:
        if has_ingredient(cereal[2], ingredient):
            cereals_with_ingredients.append(cereal)
    return cereals_with_ingredients


cereals_with_oats = get_cereals_by_ingredient(cereals[1:], 'oats') # TODO call function

# TODO Uncomment
print(f"\n4.1 Cereals by ingredient\n {cereals_with_oats}")


# 4.2 HIGHEST SUGAR CONTENT

headers = cereals[0] # extract headers

def get_cereal_attribute(cereal, headers, header):
    return cereal[headers.index(header)] # TODO implement

cereals_max_sugar = []
max_sugar_grm = 0

for cereal in cereals[1:]:
    sugar_level = get_cereal_attribute(cereal, headers, 'sugar_gm')
    if sugar_level > max_sugar_grm:
        cereals_max_sugar = []
        cereals_max_sugar.append(cereal[1])
        max_sugar_grm = sugar_level
    elif sugar_level == max_sugar_grm:
        cereals_max_sugar.append(cereal[1])
