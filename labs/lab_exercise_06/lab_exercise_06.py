...
def add_plants(plants, new_plants):
    ...
    for plant in new_plants:
        for item in plant:
            plant[plant.index(item)] = item.strip('/')
...
def count_avg_height(plants):
    ...
    height_total = 0
    for plant in plants:
        height_total += int(plant[1].strip(' feet'))

    return int(height_total/len(plants))
...
def filter_large_plants(plants, avg_height):
    ...
    large_plants = []
    for plant in plants:
        if int(plant[1].strip(" feet")) > avg_height:
            large_plants.append(plant)
    return large_plants
